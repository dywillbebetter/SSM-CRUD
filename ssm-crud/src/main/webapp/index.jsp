...
<body>
	...
	<script type="text/javascript">
		...
		function build_emps_table(result) {
			...
			$.each(emps, function(index, item) {
				...
				var editBtn = $("<button></button>").addClass(
						...
						$("<span></span>").addClass(
								"glyphicon glyphicon-paperclip")).append("编辑");
		...
		$("#emp_add_modal_btn").click(function(){
			//清除表单数据（表单完整重置，包括表单的数据和表单的样式）
			reset_form("#empAddModal form")
			//$("#empAddModal form")[0].reset();
			//发送ajax请求，获取部门信息显示在下拉列表中
			getDepts("#empAddModal select");//getDepts("#dept_add_select");
			//弹出模态框
			$("#empAddModal").modal({
				backdrop:"static"
			});
		...
		function validate_add_form(){
			...
			if(!regName.test(empName)){
				//alert("用户名需要是2-5位的中文或者6-16位的英文数字组合");
				//应该清空这个元素之前的样式
				show_validate_msg("#empName_add_input","error","用户名需要是2-5位的中文或者6-16位的英文数字组合");
				return false;
			}else{
		...
		//1.我们是在创建按钮之前就绑定了click，所以绑定不上
		//1).可以在创建按钮时绑定.2).绑定点击.live()
		//jquery新版没有live方法了，使用on进行代替
		...
		$("#emp_update_btn").click(function(){
			...
			//2.发送ajax请求保存新的员工数据+"&_method=PUT"
			$.ajax({
